...
6. The model trains, checkpoints periodically, saves a final model, evaluates episodes using the algorithm's eval policy, then records one playback video.
...
artifacts/evaluation/playback/<profile>/
  <game>/<algo>/*.mp4            one recorded episode per model/game/seed
```
...
- `python -m src.inference.record_playback <profile> <env>`: regenerate playback from a saved profile checkpoint. Use `all` for every configured game.
